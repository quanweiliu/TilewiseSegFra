...
class DE_CCFNet34_3B(nn.Module):
    def __init__(self, bands1=16, bands2=3, bands3=4, n_classes=1, classification="Multi", is_pretrained=False):
        ...
        if is_pretrained:
            ...
            sar_resnet = models.resnet34(pretrained=False)
        ...
        self.sar_first = nn.Sequential(
            ConvBNReLU(bands3, filters[0] // 2, ks=3, stride=2, padding=1),
            ConvBNReLU(filters[0] // 2, filters[0] // 2, ks=3, stride=1, padding=1),
            ConvBNReLU(filters[0] // 2, filters[0], ks=3, stride=1, padding=1),
            # nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        )
        self.sar_encoder1 = nn.Sequential(
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
            sar_resnet.layer1,
        )
        self.sar_encoder2 = sar_resnet.layer2
        self.sar_encoder3 = sar_resnet.layer3
        self.sar_encoder4 = sar_resnet.layer4

        # cross_block
        self.cross_block0 = CMF_re63(filters[0], filters[0], r=8)
        self.cross_block1 = CMF_re63(filters[0], filters[0], r=8)
        # self.cross_block0 = CMF_sp(filters[0],r=8)  #(*,64,256,256)
        # self.cross_block1 = CMF_sp(filters[0],r=8)  #(*,64,128,128)
        self.cross_block2 = CMF_re63(filters[1], filters[0], r=16)
        self.cross_block3 = CMF_re63(filters[2], filters[1], r=16)
        ...
        # )
        self.center = CMF_re63(filters[3], filters[2], r=16)
    ...
    def forward(self, x, y, z):
        ...
        ## center
        _, _, center = self.center(self.rgb_encoder4(xe3), self.lidar_encoder4(ye3), self.sar_encoder4(e3))
