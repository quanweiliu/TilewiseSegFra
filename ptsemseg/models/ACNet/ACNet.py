...
class ACNet(nn.Module):
    def __init__(self, bands1, bands2, num_class=2, pretrained=True):
        ...
        self.out5_conv=nn.Sequential(
            nn.Conv2d(256, num_class, kernel_size=1, stride=1, bias=True),
            nn.Sigmoid()
        )
        self.out4_conv=nn.Sequential(
            nn.Conv2d(128, num_class, kernel_size=1, stride=1, bias=True),
            nn.Sigmoid()
        )
        self.out3_conv=nn.Sequential(
            nn.Conv2d(64, num_class, kernel_size=1, stride=1, bias=True),
            nn.Sigmoid()
        )
    ...
    def _load_resnet_pretrained(self, weights_name='resnet50'):
        pretrain_dict = model_zoo.load_url(model_urls[weights_name])
        print("loading", weights_name)
        model_dict = {}
