...
from ptsemseg.models.segment_anything.modeling.image_encoder import ImageEncoderViT, Attention, window_partition, window_unpartition
from ptsemseg.models.segment_anything.modeling.common import MLPBlock, LayerNorm2d
...
def freeze_early_layers(module, freeze_until_block=10):
    """
    有选择性地冻结 encoder 的前几个块，保持后面的块以及所有 Adapter 可训练。
    这样既能保留 SAM 的预训练知识（前面层），又能让模型适应新任务（后面层和 Adapter）。
    
    Args:
        module: ImageEncoder 模块
        freeze_until_block (int): 冻结到第几个块（不含第 freeze_until_block 块）
                                  例如 freeze_until_block=10 时，冻结 blocks 0-9，保持 blocks 10-11 可训练
    """
    trainable_param_names = []
    frozen_param_names = []
    
    print(f'冻结 encoder 的前 {freeze_until_block} 个块，保持后续块和 Adapter 可训练...')
    for name, param in module.named_parameters():
        if 'blocks.' in name:
            # 提取 block 的索引
            block_idx = int(name.split('blocks.')[1].split('.')[0])
            if block_idx < freeze_until_block:
            ...
            # 非 block 的参数（pos_embed, patch_embed, pyramid_Adapter 等）默认可训练
            param.requires_grad = True
            trainable_param_names.append(name)
    
    print(f'冻结参数数: {len(frozen_param_names)}')
    print(f'可训练参数数: {len(trainable_param_names)}')
    print(f'被冻结的层: blocks 0-{freeze_until_block-1}')
    print(f'可训练的层: blocks {freeze_until_block}-11 + pyramid_Adapter + pos_embed + patch_embed')
...
def build_sam_adapter_linknet(
        ...
        image_size=1024,
        n_classes=6,
        freeze_strategy='all_except_adapter',
    ...
    Args:
        freeze_strategy (str): 冻结策略，可选:
            - 'all_except_adapter': 冻结除 Adapter 外的所有参数（原始策略，导致性能下降）
            - 'early_layers': 只冻结前 freeze_until_block 个块，后续块和 Adapter 可训练（推荐）
            - 'none': 不冻结任何参数
        freeze_until_block (int): 当 freeze_strategy='early_layers' 时，冻结的块数
    """
    ...
    decoder = LinkNetDecoder([prompt_embed_dim] * 4, num_classes=n_classes)
    model = EncoderDecoder(encoder, decoder)
    
    # 根据策略选择冻结方式
    if freeze_strategy == 'all_except_adapter':
        print(f"使用冻结策略: {freeze_strategy} (冻结除 Adapter 外所有参数)")
        freeze_all_except_adapter(model.enc)
    elif freeze_strategy == 'early_layers':
        print(f"使用冻结策略: {freeze_strategy} (只冻结前 {freeze_until_block} 个块)")
        freeze_early_layers(model.enc, freeze_until_block=freeze_until_block)
    elif freeze_strategy == 'none':
        print(f"使用冻结策略: {freeze_strategy} (不冻结任何参数)")
        pass
    else:
        raise ValueError(f"未知的冻结策略: {freeze_strategy}")

    return model


def build_sam_vit_b_adapter_linknet(checkpoint=None, 
                                    image_size=1024, 
                                    n_classes=6, 
                                    freeze_strategy='all_except_adapter', 
                                    freeze_until_block=10):
