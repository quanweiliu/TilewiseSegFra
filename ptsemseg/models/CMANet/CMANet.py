# https://github.com/The1912/CMANet
import torch
...
class CMANet(nn.Module):
    def __init__(self,  bands1, bands2, n_classes, classification="Multi", pretrained=True):
        ...
        # output stage
        self.out5_conv = nn.Conv2d(256, n_classes, kernel_size=1, stride=1, bias=True)
        self.out4_conv = nn.Conv2d(128, n_classes, kernel_size=1, stride=1, bias=True)
        self.out3_conv = nn.Conv2d(64, n_classes, kernel_size=1, stride=1, bias=True)
    ...
    def forward(self, rgb, dep):
        # stage1
        # rgb encoder1
        rgb1 = self.conv1(self.first_rgb(rgb))
        rgb1 = self.bn1(rgb1)
        rgb1 = self.relu(rgb1)  # (64,240,320)

        # dep encoder1
        dep1 = self.conv1_d(self.first_d(dep))
...
if __name__ == '__main__':
    ...
    print("output", output[0].shape, output[1].shape, \
          output[2].shape, output[3].shape, output[4].shape)
