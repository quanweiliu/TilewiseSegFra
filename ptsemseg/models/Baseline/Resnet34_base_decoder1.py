...
class Resnet_base34(nn.Module):
    def __init__(self, n_classes=2, is_pretrained="ResNet34_Weights.DEFAULT", data='rgb'):
...
if __name__=="__main__":
    ...
    model = Resnet_base34(data='rgb').to(device)
    output = model(x)
    print("output", output.shape)


    y = torch.randn(1, 3, 128, 128, device=device)
    model = Resnet_base34(data='lidar').to(device)
    output = model(y)
    print("output", output.shape)
