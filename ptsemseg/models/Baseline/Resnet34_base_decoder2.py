...
class Resnet_base34_re(nn.Module):
    def __init__(self, n_classes=2, is_pretrained="ResNet34_Weights.IMAGENET1K_V1", data='rgb'):
        super(Resnet_base34_re, self).__init__()
        ...
        #decoder
        self.decoder4 = decoder(filters[3], filters[2])
    ...
    def forward(self, x):
        ...
        d4 = self.decoder4(e3, center)
        d3 = self.decoder3(e2, d4)

        # print("e1", e1.shape, "d3", d3.shape)
        d2 = self.decoder2(e1, d3)
        d1 = self.decoder1(x_first, d2)
...
if __name__=="__main__":
    ...
    model = Resnet_base34_re(data='rgb').to(device)
    output = model(x)
    print("output", output.shape)


    y = torch.randn(1, 3, 128, 128, device=device)
    model = Resnet_base34_re(data='lidar').to(device)
    output = model(y)
    print("output", output.shape)
