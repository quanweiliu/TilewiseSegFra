'''
first-conv: 3* 3x3 conv 3,64-64,128
layer:1-4
cross: identity
skip_connection: +
center: +
...
class CRFN_base50(nn.Module):
    def __init__(self, n_classes=2, is_pretrained="ResNet50_Weights.DEFAULT"):
        super(CRFN_base50, self).__init__()
        ...
        self.rgb_first = nn.Sequential(
            ConvBNReLU(224, 32, ks=3, stride=2, padding=1),
            ConvBNReLU(32, 32, ks=3, stride=1, padding=1),
            ConvBNReLU(32, 64, ks=3, stride=1, padding=1),
        )
        ...
        self.lidar_first = nn.Sequential(
            ConvBNReLU(2, 32, ks=3, stride=2, padding=1),
            ConvBNReLU(32, 32, ks=3, stride=1, padding=1),
            ConvBNReLU(32, 64, ks=3, stride=1, padding=1),
        )
...
if __name__=="__main__":
    ...
    y = torch.randn(4, 2, 256, 256, device=device)

    model = CRFN_base50(n_classes=2, is_pretrained="ResNet50_Weights.DEFAULT").to(device)
    # print(model)
    output = model(x, y)
