...
class CMFNet(nn.Module):
    ...
    def __init__(self, in_channels1=4, in_channels2=2, out_channels=2):
        super(CMFNet, self).__init__()
        self.pool = nn.MaxPool2d(2, return_indices=True)
        self.unpool = nn.MaxUnpool2d(2)

        ##### RGB ENCODER ####
        self.conv1_1 = nn.Conv2d(in_channels1, 64, 3, padding=1)
        ...
        ##### DSM ENCODER ####
        self.conv1_1_d = nn.Conv2d(in_channels2, 64, kernel_size=3, padding=1)
        ...
        self.mtc = ChannelTransformer_cross(config_vit, vis, 512,
                                            channel_num=[64, 128, 256, 512],
                                            patchSize=config_vit.patch_sizes)
        self.mtc1 = ChannelTransformer(config_vit, vis, 512,
                                       channel_num=[64, 128, 256, 512],
                                       patchSize=config_vit.patch_sizes)
        ####  RGB DECODER  ####
        self.conv5_3_D = nn.Conv2d(512, 512, 3, padding=1)
    ...
    def forward(self, x, y):
        ...
        # x4t_attention = self.attention_5(x4)
        # y4t_attention = self.attention_5(y4)
        # x4t = torch.mul(x, x4t_attention)
        # y4t = torch.mul(y5, y4t_attention)
        # x4 = x4t + y4t
        x = x + x4 + xtf4
        ...
        x = self.other1(self.conv1_1_D(x))
        return x



def load_init_weight(net):
    # Download VGG-16 weights from PyTorch
    ...
    vggkeys = list(vgg16_weights.keys())
    thiskeys = list(net.state_dict().keys())
...
if __name__ == "__main__":
    ...
    in_channels2 = 1
    classes = 2
    rgb = torch.randn(4, in_channels1, 512, 512, device=device)
    dsm = torch.randn(4, in_channels2, 512, 512, device=device)

    # rgb = torch.randn(4, 224, 128, 128, device=device)
    # dsm = torch.randn(4, 3, 128, 128, device=device)

    model = CMFNet(in_channels1=in_channels1, in_channels2=in_channels2, out_channels=classes).to(device)
    # load_init_weight(model)

    output = model(rgb, dsm)
    print(output.shape)
    # rgb_parameters = [param for name, param in model.named_parameters()]
