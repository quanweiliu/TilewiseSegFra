...
class SFAFMA101(nn.Module):
    def __init__(self, bands1, bands2, n_classes, classification="Multi"):
        super(SFAFMA101, self).__init__()
        resnet_raw_model1 = models.resnet101(weights="ResNet101_Weights.DEFAULT") #preweights of backbone
        resnet_raw_model2 = models.resnet101(weights="ResNet101_Weights.DEFAULT")

        filters = [64, 256, 512, 1024, 2048]

        # gaofen
        self.encoder_rgb_conv1=nn.Sequential(
        ...
        # lidar
        self.encoder_thermal_conv1=nn.Sequential(
...
def unit_test():
    ...
    thermal = torch.randn(num_minibatch, 1, 512, 512).cuda(0)
    sfafma = SFAFMA101(2).cuda(0)
    #print(sfafma)
    #test = testm(9).cuda(0)
    #para_num_dict=get_parameter_number(test)
    #print(para_num_dict['Total'])
    input = torch.cat((rgb, thermal), dim=1)
    flops, params = profile(sfafma, inputs=(rgb,thermal))
...
if __name__ == '__main__':
    ...
    model = SFAFMA101(bands1, bands2, n_classes=1).to(device)
