...
class ISA_loader3_test(data.DataLoader):
    def __init__(self,
                 root,
                 txt_name='train.txt',
                 img_size=512,
    ...
    def __getitem__(self, index):
        ...
        gaofen2np, lidar2np = self.scaleNorm(gaofen2np, lidar2np)
        # print("mask", mask.shape)

        if self.augmentation:
            # gaofen, lidar, mask = self.is_aug(gaofen2np, lidar2np, mask)
            gaofen, lidar = self.is_aug(gaofen2np, lidar2np)
        # print("mask", mask.shape)
        # else:
    ...
    def scaleNorm(self, gaofen2np, lidar2np):
    ...
    def is_aug(self, gaofen2np, lidar2np, gaofen_band=3, lidar_band=3):
        # _, _, gaofen_band = gaofen2np.shape
        # _, _, lidar_band = lidar2np.shape
        ...
        img = torch.cat((gaofen2np, lidar2np), dim=0)  # (512,512,*)
        img = aug(img)
...
if __name__ == '__main__':
    ...
    # dataset = ISA_loader3_test(root, txt_name='train.txt', img_size=1024, classes=1, is_augmentation=True)
    dataset = ISA_loader3_test(root, txt_name='test.txt', img_size=1024, classes=1, is_augmentation=False)
    trainloader = data.DataLoader(dataset, batch_size=4, shuffle=True)

    # for gaofen, lidar, mask in trainloader:
    for gaofen, lidar in trainloader:
